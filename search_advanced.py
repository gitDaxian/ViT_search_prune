    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            torch.save(model.state_dict(),
                       'checkpoint/mini-image/advance_cs/imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth')
...
def prune(attn_ratio, ffn_ratio, flag):
    ...
    if os.path.exists('checkpoint/mini-image/advance_cs/imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth'):
        file_name = 'imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth'
        fjson = "checkpoint/mini-image/advance_cs/model_cfg_prune.json"
        # 读取原始json文件
        ...
        weight = torch.load('checkpoint/mini-image/advance_cs/'+file_name)
        ...
        file_name = 'imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth'
        file_path = 'checkpoint/mini-image/advance_cs/' + file_name
    ...
    if flag == 0: # 剪枝ffn_ratio
        ...
        file_name = 'imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + ffn_str + '.pth'
    else: # 剪枝attn_ratio
        ...
        file_name = 'imagenet_pruned_attn' + attn_str + '_ffn' + str(ffn_ratio) + '.pth'

    cfg_prune = []
    if os.path.exists('checkpoint/mini-image/advance_cs/'+file_name):
        fjson = "checkpoint/mini-image/advance_cs/model_cfg_prune.json"
        # 读取原始json文件
        with open(fjson, 'r') as f:
            content = json.load(f)
    ...
    # file_name = 'imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth'
    file_path = 'checkpoint/mini-image/advance/' + file_name
    torch.save(real_model.state_dict(), file_path)

    print(
        'imagenet_pruned_attn' + str(attn_ratio) + '_ffn' + str(ffn_ratio) + '.pth' + '==>' + getFileSize(file_path))

    return pruned_acc


# 候选区间查找
# def candidateIntervalSearch(pruned_acc, ratio, flag, flag_ratio, target, error, length):
#     """
#     @param pruned_acc:
#     @param ratio:
#     @param flag:
...
#             ratio += length
#             if flag == 1 :
#                 pruned_acc = prune(ratio, flag_ratio)
#             else :
#                 pruned_acc = prune(flag_ratio, ratio)
#         else :
#             return  (pruned_acc, ratio, flag_ratio) if flag == 1 else (pruned_acc, ratio, flag_ratio)
...
if __name__ == '__main__':
    ...
    # 导入权重文件
    model_path = "checkpoint/mini-image/advance_cs/imagenet_pruned_attn0.0_ffn0.0.pth"
